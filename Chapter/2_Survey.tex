...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{@{} l >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X @{}}
        ...
        \midrule
        \textbf{State Model} & Isolated (Siloed Liquidity) & Fragmented Pools & Unified Global State (Hub-and-Spoke) \\
        \addlinespace
        \textbf{Verification} & Native On-chain Verification & Trusted Third-Party (e.g., LayerZero Oracle) & Sovereign On-chain Verification (Solana) \\
        \addlinespace
...
\begin{itemize}
    \item \textbf{Universal Wallet Dependency:} The ``Deposit/Borrow/Repay/Withdraw'' use case has an \texttt{<<include>>} relationship with the ``Create Universal Wallet'' use case. This enforces a strict precondition: a user must establish a valid identity (Universal Wallet PDA) on Solana before performing any asset-related operations.
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item If the signature is \textbf{Invalid}, the Gateway immediately signals a failure, bypassing the core logic.
        \item If \textbf{Valid}, the request is forwarded to the Lending CDP contract.
    \end{itemize}
    ...
    \begin{itemize}
        \item \textbf{Success Path:} If a event is captured, Guardians trigger the \textit{Success Callback} on the EVM contract to finalize the action (e.g., Mint stablecoins).
        \item \textbf{Revert Path:} If the verification or logic failed, Guardians trigger the \textit{Revert Callback} to unlock the user's assets and reset the nonce, ensuring funds are never stuck.
    \end{itemize}
\end{enumerate}
