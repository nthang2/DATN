...
\begin{enumerate}[label=(\roman*)]
    \item User: The borrower who holds collateral on EVM chains. They interact with the system by Lending Operations which include depositing assets, borrowing, repaying, and withdrawing, and manage their global debt position.
...
\begin{figure}[htbp]
    ...
    \node[usecase] (uc_req)     at (4, 3.5)    {Create request \&\\Sign msg};

    \node[usecase] (uc_deposit) at (-2.5, -3.5)  {Lending\\Operations};
    \node[usecase] (uc_sign)    at (4, -3.5)   {Sign msg \&\\ Create requests};
    % Cột 3 thu về 6.5
    ...
    \draw[include] (uc_req.east) -- node[lbl, sloped] {<<include>>} (uc_listen.north);
    \draw[include] (uc_sign.east) -- node[lbl, sloped] {<<include>>} (uc_listen.south);
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        end/.style={circle, draw=black, thick, fill=black, minimum size=0.6cm, double, double distance=0.08cm},
        process/.style={
            rectangle, 
            draw=black!80, 
            thick, 
