...
\subsubsection{Function 1: Cross-chain Lending Operations (Deposit, Mint, Withdraw, Redeem)}
...
\begin{table}[H]
    ...
    \textbf{Scenario:} User redeem and withdraw max valid collateral amount. \newline 
      \textbf{Input:} \newline -  Withdraw collateral: 100 USDC \newline - Redeem: 95 UniUSD \newline - New LTV: 0\% 
    & \textbf{1.} User calls requestAction on EVM. \newline 
      \textbf{2.} Check EVM Vault balance: decreases by 100 USDC. \newline 
      \textbf{3.} Mock Guardian submits valid signature to Solana. \newline 
      \textbf{4. Expected:} Solana state updates UniversalWallet debt to zero and collateral to zero. Transaction succeeds. 
    & Passed \\ \hline

    \textbf{Scenario:} User withdraw exceeds collateral allowed \newline 
      \textbf{Input:} \newline -  Withdraw collateral: 100 USDC \newline - New LTV: 100\% 
    & \textbf{1.} User calls requestAction on EVM (this passes as EVM doesn't check LTV). \newline 
      \textbf{2.} Guarndian relays to Solana. \newline 
      \textbf{3.} Solana contract calculates projected LTV. \newline 
      \textbf{4. Expected:} Calculation shows $LTV > MaxLTV$. Transaction on Solana reverts with an error. 
    & Passed \\ \hline
